...
如果只想跑通流程，下面这段就够用了（全部从 Cargo.toml 读取元信息）：
...
A：从 `Cargo.toml` 的 `[package]` 段自动读取。映射关系：`name` ← `CARGO_PKG_NAME`，`version` ← `CARGO_PKG_VERSION`，`author` ← `CARGO_PKG_AUTHORS`，`description` ← `CARGO_PKG_DESCRIPTION`，`dependencies` 默认空数组。任何字段都可以显式写出以覆盖默认值。
