# Zpanel DSO 扩展规范 & 开发规划文档

> 本文档分两部分：
>
> **第一部分（§1–§4）：zpanel DSO 扩展的 C ABI 规范** — 语言无关，任何能编译成 C ABI 动态库的语言（Rust / C / C++ / Go / Zig / ...）都可以按此规范写扩展。
>
> **第二部分（§5 起）：Rust SDK 的设计与实现细节** — 面向使用 `zpanel-sdk` 的 Rust 开发者，解释宏展开、类型布局、ACL 机制、配置加载等内部原理。
>
> 读完本文后你应当能回答：
> 1. zpanel DSO 扩展的核心规范是什么？
> 2. 主程序怎么发现、加载、调用扩展？
> 3. 不用 SDK 能不能写扩展？需要满足什么条件？
> 4. Rust SDK 的那些宏在背后生成了什么代码？
> 5. 当前有哪些已知限制？后续往哪儿走？
...
- [2. 整体架构](#2-整体架构)
- [3. 扩展生命周期](#3-扩展生命周期)
- [4. C ABI 契约（语言无关规范）](#4-c-abi-契约语言无关规范)
- [5. Rust SDK 过程宏的展开细节](#5-rust-sdk-过程宏的展开细节)
- [6. 核心类型详解](#6-核心类型详解)
...
- 扩展可以**用任何能编译成 C ABI 动态库的语言**实现（Rust 是一等公民，C / C++ / Go / Zig 也可以）。
- 扩展与主程序之间通过**约定的 C ABI 符号**通信，而不是语言特定的接口。
...
| 概念 | 是什么 | 是否必须 |
|------|--------|----------|
| **zpanel DSO 扩展 C ABI 规范** | 一套语言无关的接口约定（符号名、JSON 格式、返回码、调用约定） | ✅ 必须满足，主程序只认这个 |
| **`zpanel-sdk`（Rust SDK）** | Rust 语言的便利工具库，帮你生成符合规范的 C 符号、提供类型定义 | ❌ 不是必须的，只是 Rust 开发者用起来最方便 |

`zpanel-sdk` 本身**不是**一个被链接进扩展的运行时——它做的事情很薄：

- 定义主程序与扩展之间传递的**类型**（`Request` / `Response` / `Method` / `Action` 等）。
- 提供一组**过程宏**（`zpanel_extension!` / `#[init]` / `#[request_hook]` 等），把扩展开发者写的 Rust 函数包装成主程序能调用的 `extern "C"` 符号。
- 提供 `Config` 加载、`ExtensionError` 统一错误等便利工具。

它不参与请求分发，也不在请求热路径上做任何额外工作——所有运行时调用都是主程序直接通过 `libloading` 调用扩展导出的 C 函数。

> **完全可以不用 SDK 写扩展**。C / C++ / Go / Zig 等语言的开发者可以直接按 §4 的 ABI 规范手写导出符号。附录 A 给出了零 SDK 依赖的 Rust 手写示例作为参考。
...
│                                       └──────────────────────────┘  │
└───────────────────────┬───────────────────────────────────────────────┘
                        │ C ABI 调用（extern "C"）—— 语言无关的规范
                        ▼
┌──────────────────────── 你的扩展 (.so / .dll / .dylib) ───────────────────┐
│                                                                          │
│  导出符号（必须满足 C ABI 规范，任何语言都可以实现）：                     │
│   zpanel_extension_get_meta     ← 返回元信息 JSON                        │
│   zpanel_extension_init         ← 初始化                                  │
│   zpanel_extension_start        ← 启动                                    │
│   zpanel_extension_stop         ← 停止                                    │
│   zpanel_extension_on_request   ← 请求钩子                                │
│   zpanel_extension_on_response  ← 响应钩子                                │
│   <acl_fn> + zpanel_acl_name_<acl_fn>  ← ACL 模块（可选）                │
│                                                                          │
│  ────────────────────────────────────────────────────────────────────── │
│  |  如果用 Rust SDK，以下由 SDK 自动生成，开发者只需写业务代码            | │
│  |   zpanel_extension!()    →  生成 get_meta（自动读 Cargo.toml）       | │
│  |   #[init]/#[request_hook] →  包装 Rust 函数为 extern "C" 符号        | │
│  ────────────────────────────────────────────────────────────────────── │
│                                                                          │
│   可选依赖：zpanel_sdk（仅类型 + 宏，无额外运行时）                      │
└──────────────────────────────────────────────────────────────────────────┘
```
...
- **C ABI 是规范，SDK 是便利工具**：任何语言只要导出约定符号就能写扩展。Rust SDK 只是帮你少写样板代码。
...
一个扩展从被 zpanel 发现到被卸载，会经历以下阶段。每个阶段对应一个 C ABI 符号——用任何语言实现这些符号都可以。

> 使用 Rust SDK 的开发者，这些符号由 `#[init]` / `#[start]` / `#[stop]` 等宏自动生成，你只需要写带对应属性的 Rust 函数。
...
这是**最重要的章节**——zpanel DSO 扩展的核心规范。任何语言（Rust / C / C++ / Go / Zig / ...）写的扩展，只要满足以下 ABI 契约，就能被 zpanel 主程序识别和加载。

> 使用 Rust SDK 的开发者：这些符号和返回码由 SDK 的过程宏自动处理，你一般不需要关心底层细节。但了解它们有助于调试和理解 SDK 背后做了什么。
...
> 本节及之后的内容是 **Rust SDK 专属**的设计细节。用其他语言写扩展的读者可以跳过本节，直接看 [附录 A](#附录-a手写一个不依赖-sdk-的扩展) 了解如何手写 C ABI 导出符号。
>
> 如果你用 Rust SDK 写扩展，本节会帮你理解每个宏背后生成了什么代码、有哪些隐藏行为、为什么某些限制存在。
